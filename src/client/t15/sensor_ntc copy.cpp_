...
static void sample_hotspot_unavailable()
{
    // Keep Hotspot structurally present for future use.
    // At the moment it is intentionally marked invalid/unavailable.
    g_sample_next.rawHotspot = g_ads.readADC_SingleEnded(0);
    g_sample_next.hot_mV = ntc::ads_raw_to_mV(g_sample_next.rawHotspot);
    g_sample_next.hot_ohm = -1;
    g_sample_next.hotValid = false;
    g_sample_next.hot_dC = ntc::TEMP_INVALID_DC;
    g_sample_next.tempHotspotC = NAN;
}
...
void sample_chamber_temperature()
{
    ...
    // Start from current stable sample so fields not touched in this cycle stay consistent.
    g_sample_next = g_sample;

    sample_hotspot_unavailable();
    ...
    // Keep ADS state stable across commits.
    g_sample_next.adsOk = g_sample.adsOk;

    // Atomic snapshot commit for readers.
