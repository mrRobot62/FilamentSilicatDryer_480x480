...
void HostComm::handleIncomingLine(const String &line) {
    ...
    case ProtocolMessageType::ClientAckSet:
        ...
        DBG("ACK UPD received, new mask=0x%04X\n", mask);
        _lastSetAcked = true;
    ...
    case ProtocolMessageType::ClientAckUpd:
        // Client confirmed an UPD command; mask is the new outputs mask
        _remoteStatus.outputsMask = mask;
        break;

    case ProtocolMessageType::ClientAckTog:
        // Client confirmed a TOG command; mask is the new outputs mask
        _remoteStatus.outputsMask = mask;
        break;

    case ProtocolMessageType::ClientRst:
        // Client reported a reset. Accept it without marking comm error.
        // Optionally clear cached flags/state to avoid stale status.
        WARN("Client reported reset\n");
        _newStatus = false;
        _lastSetAcked = false;
        break;
    ...
    default:
        ...
        ERR("Unexpected protocol message type=%d\n", (int)type);
        _commError = true;
        break;
    }
}
