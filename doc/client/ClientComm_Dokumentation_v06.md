...
### CLIENT SafetyGuard (T14.0)

Ein dedizierter **SafetyGuard‑Mechanismus** wurde vollständig auf der
CLIENT‑Seite implementiert, um sicherzustellen, dass der HEATER
ausschließlich unter expliziter Kontrolle des HOST aktiv sein kann.

#### Sicherheitsziele

Der CLIENT garantiert nun deterministisch:

-   Default‑Safe Zustand nach Boot (Outputs = OFF)
-   Sofortige Abschaltung bei Kommunikationsverlust
-   Sofortige Abschaltung bei Protokoll‑ oder Parserfehlern
-   Sofortige Abschaltung bei unerwarteten oder ungültigen Frames
-   Keine automatische Wiederaktivierung ohne expliziten HOST‑Befehl

Dies verhindert jegliche unkontrollierte oder unbeabsichtigte
Aktivierung sicherheitskritischer Aktoren.

#### Safe‑State Trigger

Der CLIENT wechselt sofort in den SAFE‑STATE bei:

-   Host Timeout (`CLIENT_HOST_TIMEOUT_MS` überschritten)
-   Parse‑Fehler (`ProtocolCodec::parseLine` fehlgeschlagen)
-   RX‑Buffer Overflow
-   Unerwartete oder ungültige Frame‑Typen
-   Empfang eines `HostRst` Frames
...
_onOutputsChanged(0x0000);
_safetyLatched = true;
```

Der HEATER und alle sicherheitskritischen Aktoren werden garantiert
deaktiviert.

#### Safety‑Latch Mechanismus

Nach einem Safe‑State bleibt der CLIENT im gelatchten Zustand:

``` cpp
_safetyLatched == true
```

Eine Wiederaktivierung ist ausschließlich möglich durch:

-   HostSet
-   HostUpd
-   HostTog

Folgende Frames können den Safe‑State NICHT verlassen:

-   HostPing
-   HostGetStatus
-   beschädigte oder unerwartete Frames

Dies stellt sicher, dass nur explizite HOST‑Kommandos Aktoren aktivieren
können.

#### Watchdog‑Integration

Der CLIENT implementiert einen internen Kommunikations‑Watchdog:

``` cpp
g_lastHostGoodMs
```

Dieser wird nur durch gültige HOST‑Frames aktualisiert.

Bei Timeout erfolgt automatisch:

``` cpp
enterSafeState_(ClientSafetyReason::HostTimeout);
```

#### SafetyReason Reporting

Der CLIENT speichert Diagnoseinformationen zum letzten Safe‑Event:

``` cpp
bool     _safetyLatched;
uint8_t  _lastSafetyReason;
uint32_t _lastSafetyMs;
```
...
uint8_t  lastSafetyReasonRaw() const;
const char* lastSafetyReasonStr() const;
uint32_t lastSafetyMs() const;
```

Beispiel Log:

    [CLIENT][SAFETY] ENTER SAFE STATE reason=HostTimeout (outputsMask=0)

#### Determinismus und Autorität

Der CLIENT bleibt die alleinige autoritative Instanz für physische
Outputs.

Der HOST kann keine Aktoren logisch „erzwingen", wenn:

-   Kommunikationsverlust vorliegt
-   SafetyLatch aktiv ist
-   oder ein Protokollfehler aufgetreten ist

Dies erfüllt vollständig die SafetyGuard‑Architekturanforderungen von
T14.0.
...
### Protocol -- Dual‑NTC Architecture

-   STATUS frame extended to support two independent NTC sensors:
    -   `tempHotspot_dC` (Hotspot temperature, safety monitoring)
    -   `tempChamber_dC` (Chamber temperature, control and UI)
-   ADC fields `adc0..adc3` are now strictly defined as **RAW ADS1115
    conversion values**
    -   Signed integer (−32768..32767)
    -   No scaling applied
    -   ADS1115 Gain setting: ±6.144 V (LSB ≈ 0.1875 mV)
-   Temperature values are transmitted as **decimal integer in
    deci‑degrees Celsius**
    -   Example: `509` = 50.9 °C
-   Removed ambiguity of previous `temp` field (now explicitly split
    into hotspot and chamber)

### Behavioral clarification

-   Host must use `tempChamber_dC` as the sole control temperature
-   Host must use `tempHotspot_dC` exclusively for safety supervision
-   `adc0..adc3` are telemetry/debug channels only, not interpreted by
    the protocol layer
...
**Role:** CLIENT-side UART protocol handler (ESP32-WROOM) for a
Host↔Client link (ESP32-S3 HOST)

------------------------------------------------------------------------

## Ziel und Kontext

`ClientComm` kapselt die komplette **UART-Link-Kommunikation** auf der
CLIENT-Seite:
...
-   TX: Antworten an den HOST über `Serial2` (oder konfigurierbares
    `HardwareSerial`)

Wichtig: `ClientComm` ist **non-blocking** und darf permanent in
`loop()` laufen.
...
### Client → Host
  ...
  Frame                                                       Bedeutung
  ----------------------------------------------------------- --------------------------
  `C;ACK;SET;FFFF`                                            ACK für SET (übernommene
                                                              Maske)

  `C;ACK;UPD;FFFF`                                            ACK für UPD (resultierende
                                                              Maske)

  `C;ACK;TOG;FFFF`                                            ACK für TOG (resultierende
                                                              Maske)

  `C;STATUS;mask;a0;a1;a2;a3;tempHotspot_dC;tempChamber_dC`   Statuspayload (Dual‑NTC)
...
## STATUS Frame -- Detailed Definition (V0.5)
...
Field definitions:
  ...
  ---------------- ------------ ------------ ---------------------------
  mask_hex         hex uint16   bitmask      Current active output/state
                                             mask

  adc0..adc3       int16        raw counts   Direct ADS1115 conversion
                                             values (Gain ±6.144 V, no
                                             scaling)

  tempHotspot_dC   int16        °C×10        Hotspot temperature (safety
                                             monitoring)

  tempChamber_dC   int16        °C×10        Chamber temperature
                                             (control and UI)
  ----------------------------------------------------------------------

Example:

    C;STATUS;0035;12345;11890;0;-12;509;372

Meaning:

-   mask = 0x0035
-   adc0 = 12345 raw ADS1115 counts
-   adc1 = 11890 raw ADS1115 counts
-   tempHotspot = 50.9 °C
-   tempChamber = 37.2 °C
...
Host stürzt ab / wird neu geflasht, Client läuft weiter, HEATER bleibt
sonst aktiv
...
> **Host-Abstürze, Tür-Öffnungen, Kommunikationsausfälle und
> Initialisierungsfehler** abgesichert.
>
> Der Client kann den Ofen jederzeit selbstständig in einen **sicheren
> Zustand** überführen --\
...
-   **Robuste Verarbeitung eingehender Zeilen**
    ...
    -   **Wichtig:** Nach `handleIncomingLine(line)` wird **nicht**
        `return` gemacht, sondern weiter RX gepumpt (`continue`)\
-   Stabilität bestätigt durch umfangreiche HOST-TestCases

#### Fazit

`ClientComm` V0.3 ist ein **robuster, non-blocking UART
Client-Handler**, der:
...
-   `void processLine(const String &line)` (Test/Simulation: bypass
    UART)
...
4.  **Keine frühe Rückkehr**\
    Nach Verarbeitung einer Zeile wird weiter gelesen (kein `return`),
    damit Burst-RX korrekt verarbeitet wird.
...
`handleIncomingLine()` delegiert das Parsing vollständig an
`ProtocolCodec::parseLine(...)`.

-   Bei Parse-Fehler:
    -   Zeile wird geloggt (`RAW("[CLIENT] Failed to parse line: ...")`)
    -   Keine ERR-Frame-Rücksendung (bewusst minimal; optional
        erweiterbar)
...
In früheren Iterationen war ein `return` nach Zeilenverarbeitung
kritisch, weil dann bei Burst-RX nur die erste Zeile verarbeitet wurde.\
V0.3 nutzt konsequent `continue`, um RX vollständig zu konsumieren.

### 2) "Junk"-Bytes / Boot Noise

Auf echten UART-Verbindungen können am Anfang der Kommunikation
Steuerzeichen/Junk auftreten.\
V0.3 entfernt führenden Junk bis `H` oder `C`. Das hat sich in den
TestCases als stabil erwiesen.

### 3) Heartbeat nur bei Activity

`_heartBeatCb()` wird **nur** aufgerufen, wenn RX Activity vorhanden war
(`hadActivity == true`).\
...
Ein kritisches Fehlerszenario tritt auf, wenn: - der **HOST (ESP32-S3)**
abstürzt, neu geflasht oder blockiert - der **Client weiterläuft** -
zuvor **HEATER = ON** gesetzt war

Ohne zusätzliche Schutzmechanismen würde der HEATER **unbegrenzt
weiterlaufen**.
...
-   **Jedes gültige Protokoll-Frame** vom Host gilt als *Lebenszeichen*
    ...
    -   GET STATUS
    -   PING
-   Bleibt dieses Lebenszeichen zu lange aus, wird ein **Hard-Fail-Safe
    ausgelöst**
...
Beim **ersten START nach Boot** wurde im UI zwar RUNNING angezeigt, aber
der **CLIENT schaltete den HEATER nicht zuverlässig ein** -- am
Oszilloskop war **kein 4 kHz PWM-Signal** sichtbar.\
Erst nach **STOP → START** wurde der PWM-Ausgang korrekt aktiv.

**Risiko:** Anwender verlässt sich auf UI-Zustand, reale Heizleistung
bleibt aus (funktional kritisch) -- außerdem ist die Stelle
sicherheitsrelevant, weil PWM-Ansteuerung deterministisch sein muss.
...
-   nach **STOP**, kurzer Pause, **START (2. Mal)** → PWM sofort
    sichtbar

Client-Log zeigte dabei: - SET;0051 wird empfangen und ACK gesendet -
„PWM attached ..." wird geloggt - dennoch war beim ersten Start am Oszi
kein Signal (sporadisch / initial)

#### Ursache (Root Cause)

Im Zusammenspiel aus **LEDC Attach/Write** und dem initialen
GPIO/LEDC-Zustand konnte der **erste Duty-Write nach Attach** unter
bestimmten Bedingungen „verloren" gehen bzw. nicht deterministisch am
Pin ankommen (Timing/GPIO-Matrix/LEDC-Startphase).

#### Fix / Patch

Die Funktion `heaterPwmEnable(true)` wurde so angepasst, dass der
PWM-Start **deterministisch** wird: - definierter „Kick"-Start -
sofortiges Setzen eines bekannten Duty-States (0 → Ziel-Duty) - kurze
`delayMicroseconds()`-Stabilisierung - optionaler zweiter Duty-Write
(harmlos, aber robust gegen „first-write lost")

> Ergebnis: **1. START nach Boot** liefert zuverlässig 4 kHz PWM am
> Oszilloskop, ohne STOP/START Workaround.
...
-   PWM-Ansteuerung auf ESP32 sollte **nicht nur logisch korrekt**,
    sondern auch **elektrisch deterministisch** sein.
-   Bei sicherheitsrelevanten Aktoren (HEATER) ist „first-run
    reliability" Pflicht, sonst entstehen schwer zu diagnostizierende
    Feldfehler.
