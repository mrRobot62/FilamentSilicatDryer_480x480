...
// ----------------------------------------------------------------------------
// Helper: normalize a built protocol line to match UART RX behavior
// - ProtocolCodec::build*() returns strings with CRLF.
// - ClientComm::loop() strips '\r' and terminates on '\n', so handleIncomingLine()
//   always receives a line WITHOUT CR/LF.
// ----------------------------------------------------------------------------
static String normalizeLine_(String s) {
    s.replace("\r", "");
    s.replace("\n", "");
    s.trim();
    return s;
...
static void test_unexpected_frame_enters_safe() {
    ...
    const String line = normalizeLine_(ProtocolCodec::buildClientPong()); // e.g. "C;PONG"
...
static void test_host_rst_enters_safe() {
    ...
    cc.handleIncomingLine(normalizeLine_(ProtocolCodec::buildHostRst()));
...
static void test_latch_not_cleared_by_ping_or_getstatus() {
    ...
    cc.handleIncomingLine(normalizeLine_(ProtocolCodec::buildHostPing()));
    TEST_ASSERT_TRUE_MESSAGE(cc.isSafetyLatched(), "PING must not clear safety latch");

    // GET STATUS must NOT clear latch.
    cc.handleIncomingLine(normalizeLine_(ProtocolCodec::buildHostGetStatus()));
...
static void test_latch_cleared_only_by_set_upd_tog() {
    ...
    cc.handleIncomingLine(normalizeLine_(ProtocolCodec::buildHostSet(desired)));
    ...
    cc.handleIncomingLine(normalizeLine_(ProtocolCodec::buildClientPong()));
    TEST_ASSERT_TRUE(cc.isSafetyLatched());
    TEST_ASSERT_EQUAL_UINT16(0x0000, cc._outputsMask);

    // UPD must clear latch and update mask.
    cc.handleIncomingLine(normalizeLine_(ProtocolCodec::buildHostUpd(/*set=*/0x0002, /*clr=*/0x0000)));
    ...
    cc.handleIncomingLine(normalizeLine_(ProtocolCodec::buildHostTog(/*tog=*/0x0002)));
...
void setup() {
    ...
    UNITY_BEGIN();
    INFO("CLIENT Testcases <start> ---------------------------------------------\n");
    udp_log::send_cstr("CLIENT Testcases <start> ---------------------------------------------\n");
    ...
    udp_log::send_cstr("CLIENT Testcases < rnf> ---------------------------------------------\n");
    INFO("CLIENT Testcases <end> ----------------------------------------------\n\n");
    UNITY_END();
}
d void loop() {}
